...
const gecko = !ie$1 && /gecko\/(\d+)/i.test(nav.userAgent);
const chrome$1 = !ie$1 && /Chrome\/(\d+)/.exec(nav.userAgent);
const webkit = "webkitFontSmoothing" in doc.documentElement.style;
const safari = !ie$1 && /Apple Computer/.test(nav.vendor);
...
const baseTheme$7 = buildTheme("." + baseThemeID, {
    "&.cm-editor": {
...
const gotoLine = view => {
    ...
    if (!panel) {
        ...
        if (view.state.field(dialogField, false) == null)
            effects.push(StateEffect.appendConfig.of([dialogField, baseTheme$3]));
        view.dispatch({ effects });
...
Prec.highest(keymap.compute([snippetKeymap], state => state.facet(snippetKeymap)));
EditorView.domEventHandlers({
